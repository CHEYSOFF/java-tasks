...
public class TextFilterManager {
    ...
    public FilterType analyze(String text) {
        if (text == null || text.length() == 0 || filters.length == 0) {
            return FilterType.GOOD;
        }
        FilterType[] filtersResult = new FilterType[filters.length];
        for (int i = 0; i < filters.length; i++) {
            filtersResult[i] =  filters[i].Analyze(text);
        }
        Arrays.sort(filtersResult);
        return filtersResult[0];
    }
}
